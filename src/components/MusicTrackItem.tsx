...
export function MusicTrackItem({
  ...
  return (
    <div
      ...
      tabIndex={0}
      role="button"
      aria-label={`${track.name} - ${track.artist?.join(", ")}`}
      onKeyDown={(e) => {
        if (e.key === "Enter" || e.key === " ") {
          e.preventDefault();
          showCheckbox ? onSelect?.() : onPlay();
        }
      }}
      className={cn(
        "group grid gap-4 items-center px-4 py-2.5 transition-all text-sm cursor-pointer",
        "grid-cols-[1.75rem_1fr_auto]",
        "focus-visible:outline-2 focus-visible:outline-primary focus-visible:outline-offset-1 focus-visible:bg-muted/60 rounded",
